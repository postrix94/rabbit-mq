...
try {
    ...
    $channel->basic_qos(0, $config['queries']['b2c_lead']['prefetch_count'], false);
...
// Без подтверждения о доставке!!!
    $channel->basic_consume(
        $config['queries']['b2c_lead']['name'],
        '',
        false,
        $config['queries']['b2c_lead']['no_ack'],
        false,
        $config['queries']['b2c_lead']['nowait'],
        $callback);
